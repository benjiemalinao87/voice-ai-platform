    ...
    try {
      ...
      if (url.pathname.startsWith("/api/calls/") && url.pathname.endsWith("/control/say") && request.method === "POST") {
        ...
        try {
          ...
          if (!controlResponse.ok) {
            ...
            return jsonResponse2({ error: `Control request failed: ${error}` }, controlResponse.status);
          }
          return jsonResponse2({ success: true });
        } catch (error) {
          console.error("[Call Control] Error sending say command:", error);
          return jsonResponse2({ error: "Failed to send message" }, 500);
        }
      }
      if (url.pathname.startsWith("/api/calls/") && url.pathname.endsWith("/control/add-message") && request.method === "POST") {
        ...
        try {
          ...
          return jsonResponse2({ error: "Failed to add message" }, 500);
        }
      }
      if (url.pathname === "/api/speech-to-text" && request.method === "POST") {
        ...
        try {
          ...
          if (!audioFile) {
            return jsonResponse2({ error: "No audio file provided" }, 400);
          }
          ...
          if (!deepgramResponse.ok) {
            ...
            return jsonResponse2({ error: `Transcription failed: ${error}` }, deepgramResponse.status);
          }
          ...
          return jsonResponse2({ error: "Failed to transcribe audio" }, 500);
        }
      }
