...
import { Bot, Volume2, Power, Settings, ChevronRight, Phone, Plus, Trash2, Workflow, BarChart3 } from 'lucide-react';
...
export function VoiceAgentsList({ agents, onSelectAgent, onCreateAgent, onDeleteAgent }: VoiceAgentsListProps) {
  ...
  const handleViewAnalytics = (e: React.MouseEvent, agentId: string) => {
    e.stopPropagation(); // Prevent triggering onSelectAgent
    window.location.href = `/assistants?agent=${agentId}`;
  };
  if (agents.length === 0) {
    return (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={(e) => handleViewAnalytics(e, agent.id)}
                  className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
